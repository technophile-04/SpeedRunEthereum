...
function SkillTree({ data, title, description, closedByDefault, treeId, savedData, handleSave, handleNodeSelect, collapsible = false, disabled = false, }) {
    const isMobile = useMobile();
    const initialVisibility = closedByDefault || disabled ? false : true;
    const [isVisible, setVisibility] = useState(initialVisibility);
    const memoizedToggleVisibility = useCallback(function toggleVisibility() {
        if (disabled) {
    ...
    return (<React.Fragment>
      ...
      <SkillTreeProvider treeId={treeId} savedData={savedData} handleSave={handleSave} sendNodeSelectDataToClient={handleNodeSelect}>
        <CalculateNodeCount data={data}/>
        <SkillTreeContainer>
          <SkillTreeHeader isVisible={isVisible} disabled={disabled} handleClick={memoizedToggleVisibility} collapsible={collapsible} id={treeId} description={description} title={title}/>
        ...
        return (<React.Fragment key={skill.id}>
                    <SkillTreeSegment shouldBeUnlocked skill={skill} hasParent={false} parentPosition={0} parentHasMultipleChildren={false}/>
